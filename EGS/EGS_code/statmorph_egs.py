...
#我的修改
#as: 将计算sky_box 的区域扩展到外面而不是里面，初始大小为64，change sky_box:1416，1438, sky_a: 1486, sky_s:1500
#1405 CAS 改变sky_box
...
class SourceMorphology(object):
    ...
    def __init__(self, image, segmap, label, mask=None, weightmap=None,
                 ...
                 eta=0.2, petro_fraction_gini=0.2, skybox_size=64,
    ...
    def m20_c(self):
        ...
        #yc = M[1, 0] / M[0, 0]
        #xc = M[0, 1] / M[0, 0]
        xc = self._asymmetry_center[0]
        yc = self._asymmetry_center[1]
    ...
    @lazyproperty
    def snr(self):
        L_tot = self.flux_ellip
        q = self.elongation_asymmetry
        R_p = self._petro_extent_flux * self.rpetro_ellip
        sigma_sky = self.sky_sigma
        SNRp = L_tot/(np.pi*R_p**2/q*sigma_sky)
        return SNRp

    @lazyproperty
    def flux_ellip2(self):
        """
        Return the sum of the pixel values over an elliptical aperture
        with "radius" equal to ``petro_extent_flux`` (usually 2) times
        the elliptical Petrosian "radius".
    ...
    @lazyproperty
    def snr2(self):
        L_tot = self.flux_ellip2
        q = self.elongation_asymmetry
        R_p = self.rpetro_ellip
        #if R_p <=5:
            #print('Petrosian Radius < 5, maybe not right.')
        sigma_sky = self.sky_sigma
        SNRp = L_tot/(np.pi*R_p**2/q*sigma_sky)
        return SNRp    
    ##################
...
#change box 后缀加了sky
    @lazyproperty
    def _mask_stamp_nan_sky(self):
        """
        Flag any NaN or inf values within the postage stamp.
        """
        locs_invalid = ~np.isfinite(self._image)
        if self._weightmap is not None:
            locs_invalid |= ~np.isfinite(self._weightmap)
        return locs_invalid

    @lazyproperty
    def _mask_stamp_badpixels_sky(self):
        """
        Flag badpixels (outliers).
        """
        self.num_badpixels = -1
        badpixels = np.zeros((self.ny_stamp, self.nx_stamp), dtype=np.bool8)
        if self._n_sigma_outlier > 0:
            badpixels = self._get_badpixels(self._image)
            self.num_badpixels = np.sum(badpixels)
        return badpixels
    ...
    def _mask_stamp_sky(self):
        ...
        but the background (segmap == 0) is left alone.
        """
        segmap_stamp = self._segmap.data
        mask_stamp = (segmap_stamp != 0) & (segmap_stamp != self.label)
        if self._mask is not None:
            mask_stamp |= self._mask
        mask_stamp |= self._mask_stamp_nan_sky
        mask_stamp |= self._mask_stamp_badpixels_sky
        return mask_stamp    
    
    @lazyproperty
    def _cutout_stamp_maskzeroed_sky(self):
        """
        Return a data cutout with its shape and position determined
        by ``_slice_stamp``. Pixels belonging to other sources
        (as well as pixels where ``mask`` == 1) are set to zero,
        ...
        as well as badpixels (outliers).
        """
        return np.where(~self._mask_stamp_sky,
                        self._image, 0.0)           
        ...
        while True:
            for i in range(0, ny - cur_skybox_size):
                for j in range(0, nx - cur_skybox_size):
                    ...
                    if np.all(segmap[boxslice] == 0) and np.all(~mask[boxslice]) and \
                    not np.any(self._cutout_stamp_maskzeroed_sky[boxslice] == 0):
                        
                        return boxslice

            # If we got here, a skybox of the given size was not found.
            if cur_skybox_size <= 6:
                warnings.warn('[skybox] Skybox not found.', AstropyUserWarning)
                print('[skybox] Skybox not found.')
                self.flag = 1
                return slice(0, 0), slice(0, 0)
            else:
                cur_skybox_size = int(cur_skybox_size / 1.5)
                if self._verbose:
    ...
    def sky_mean(self):
        ...
        """
        bkg = self._cutout_stamp_maskzeroed_sky[self._slice_skybox]
        if bkg.size == 0:
            assert self.flag == 1
            return -99.0
    ...
    @lazyproperty
    def _asymmetry_center_sky(self):
        """
        ...
        bkg = self._cutout_stamp_maskzeroed_sky[self._slice_skybox]
        #maxf = bkg.shape[0]/2 + 1
        center_0 = np.array([bkg.shape[0]/2, bkg.shape[0]/2])  # initial guess
        center_asym = opt.fmin_bfgs(self._asymmetry_function_sky, center_0,
                               args=(bkg,),disp=0)
        #print('sky center = ', center_asym)
        return center_asym    

    def _asymmetry_function_sky(self, center, image):
        """
        Helper function to determine the sky asymmetry and center of asymmetry.
        ...
        mask = self._mask_stamp_sky[self._slice_skybox]
        ...
        r = min(xc,yc,nx-xc,ny-yc)
        #print(xc)
        #print(yc)
        #print('---')
        #if abs(nx/2-xc) >= 8 or abs(ny/2-yc) >= 8:
            #print('asysmetry is not right')
        ap = photutils.CircularAperture(center, r)
    ...
    def _sky_asymmetry(self):
        ...
        asym = self._asymmetry_function_sky(self._asymmetry_center_sky,
                                        bkg)
        #psrint('a_s:',self._sky_asymmetry)
        return asym#np.sum(np.abs(bkg_180 - bkg)) / float(bkg.size)
    ...
    def _asymmetry_function(self, center, image, kind):
        ...
        if kind == 'shape':
            ...
            if self._sky_asymmetry == -99.0:  # invalid skybox
                ...
                #print('_____________')
                a_asy = ap_abs_diff/ap_area
                #print('a_i:',a_asy,'   a_s:',self._sky_asymmetry)
                #print(ap_area)
                #print(ap_abs_sum)
